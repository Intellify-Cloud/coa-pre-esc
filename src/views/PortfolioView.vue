    ...
    <section class="page-hero py-16 md:py-24">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-4xl md:text-5xl font-bold tracking-tight">
          {{ page.header.title_prefix
          }}<span class="gradient-text">{{ page.header.title_highlight }}</span>
        </h1>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 split-layout">
        <aside class="plan-card">
          ...
          <div class="usp">
            ...
            <p class="mt-1">
              {{ page.plan.usp_description_prefix
              }}<s>{{ page.plan.usp_description_strikethrough }}</s
              >{{ page.plan.usp_description_suffix }}
            </p>
          </div>
          <p>{{ page.plan.description }}</p>
          <RouterLink
            :to="page.plan.button_link"
        ...
        <div>
          ...
          <div class="portfolio-tier-grid">
            <article
              v-for="tier in page.tiers"
              :key="tier.name"
              class="portfolio-tier-card"
    ...
    meta: [
      {
        name: 'description',
        content:
          'Managed monthly website subscriptions from Intellify. No upfront costs, domain registration, hosting, SEO and cloud management included. From R150/month.',
      },
    ...
    script: [
      {
        ...
        children: JSON.stringify({
          ...
          description:
            'Professional website with no upfront costs, includes hosting, domain, and SEO optimization',
          brand: {
            '@type': 'Brand',
            name: 'Intellify',
          },
...
<style scoped>
  ...
  .plan-card {
    ...
    box-shadow:
      0 14px 52px rgba(15, 23, 42, 0.22),
  ...
  .portfolio-tier-card.is-featured {
    ...
    box-shadow:
      0 16px 52px rgba(234, 88, 12, 0.18),
      inset 0 1px 0 rgba(255, 255, 255, 0.1);
  }

  .portfolio-tier-card.is-featured:hover {
    box-shadow:
      0 24px 68px rgba(234, 88, 12, 0.28),
